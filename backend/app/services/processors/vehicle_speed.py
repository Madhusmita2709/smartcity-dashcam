...
class VehicleSpeedEstimator:
    ...
    def run(self, input_video, output_dir, video_id, config_path):
        ...
        while cap.isOpened():
            ...
            for r in results:
                ...
                for box in r.boxes:
                    ...
                    #print(f"IMG=({cx},{cy})  ->  BEV=({bev[0]:.1f}, {bev[1]:.1f})")
                    bx = int(bev[0])
                    by = int(bev[1])
                    ...
                    if old_index is not None:
                        ...
                        dt = new_t - old_t
                        #print(f"Track={track_id}, dt={dt:.3f}")
                        if dt > 0:
                            #print("Entered speed calculation")
                            dx = new_x - old_x
                            dy = new_y - old_y
                            # Ignore tracker jitter
                            ...
                            f"ID={track_id} | "
                            f"Ego={ego_speed:.1f} | "
                            f"Relative={speed_kmh:.1f} | "
                            f"Vehicle={absolute_speed:.1f} km/h"
                            )
                            ...
                            if absolute_speed > speed_limit and track_id not in overspeed_ids:
                                ...
                                plate_results = self.plate_reader.read_plate(
                                original_frame,
                                track_key
                                )
